...
final class NetworkManagerTests: XCTestCase {
    func test_getVehicles_Success() async throws {
        ...
        let vehicleList: [SelectDestinations.LoadVehicles.Response] = [.mockAny(), .mockAny(), .mockAny()]
    ...
    func test_getPlanets_Success() async throws {
        let response: HTTPURLResponse = .mockResponseWith(statusCode: 200)
        let planetList: [SelectDestinations.LoadPlanets.Response] = [.mockAny(), .mockAny(), .mockAny()]
    ...
    func test_findFalcone_Success_it_returns_planetName() async throws {
        let response: HTTPURLResponse = .mockResponseWith(statusCode: 200)
        let findFalcon: FindFalcone.Find.Response = .mockRandom(planetName: .mockAny(), status: "success")
        let apiClient = APIClientMock(response: response, data: try? JSONEncoder().encode(findFalcon))
        let networkManager = NetworkManager(apiClient: apiClient, responseHandler: ResponseHandler())
        
        let expectedResponse = try await networkManager.findFalcone(FindFalcone.Find.Request(token: .mockAny(), planetNames: [.mockAny()], vehicleNames: [.mockAny()]))

        XCTAssertEqual(findFalcon, expectedResponse.value, "Falcon found")
    }
    
    func test_findFalcone_Success_it_doesn_not_return_planetName() async throws {
        let response: HTTPURLResponse = .mockResponseWith(statusCode: 200)
        let findFalcon: FindFalcone.Find.Response = .mockRandom(planetName: nil, status: "false")
        let apiClient = APIClientMock(response: response, data: try? JSONEncoder().encode(findFalcon))
        let networkManager = NetworkManager(apiClient: apiClient, responseHandler: ResponseHandler())
    ...
    func test_findFalcone_Failure() async {
        let response: HTTPURLResponse = .mockResponseWith(statusCode: 500)
        let apiClient = APIClientMock(response: response)
        let networkManager = NetworkManager(apiClient: apiClient, responseHandler: ResponseHandler())
        
        do {
            _ = try await networkManager.findFalcone(FindFalcone.Find.Request(token: .mockAny(), planetNames: [.mockAny()], vehicleNames: [.mockAny()]))
            XCTFail("Server should return error")
        } catch {
            XCTAssertTrue(true, "Server returns error")
        }
    ...
    func test_findFalcone_Failure_it_needs_retry() async {
    ...
    func test_retrieveToken_Success() async throws {
        let response: HTTPURLResponse = .mockResponseWith(statusCode: 200)
        let token: SelectDestinations.RetrieveToken.Response = .mockAny()
